...
const TrialPacksSection = () => {
  const navigate = useNavigate();
  const { products, loading } = useProducts({ category: 'Trial Pack' });

  const trialPackProducts = useMemo(() =>
    products.filter(product => product.category === 'Trial Pack'),
    [products]
  );
  ...
  return (
    <section className="py-12 lg:py-20 bg-gradient-to-br from-background to-amber-50/20">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6">
        {/* Section Header */}
        <div className="flex justify-between items-center mb-8 lg:mb-12">
          <div>
            <h2 className="text-2xl sm:text-3xl lg:text-4xl font-display font-bold text-foreground mb-2">Trial Packs</h2>
            <p className="text-sm sm:text-base text-muted-foreground max-w-2xl">
              Not sure which tea suits your palate? Explore our best-selling trial packs and find your favorite flavor profile before going big.
            </p>
          </div>
        ...
        <div className="relative">
          {loading ? (
            <div className="py-12 text-center text-amber-600 font-semibold">Loading trial packs...</div>
          ) : trialPackProducts.length === 0 ? (
            <div className="py-12 text-center text-gray-500">No trial packs found.</div>
          ) : (
            <Carousel className="w-full" opts={{ align: "start", loop: true }}>
              <CarouselContent className="-ml-2 md:-ml-4">
                {trialPackProducts.map((product) => (
                  <CarouselItem key={product.id} className="pl-2 md:pl-4 basis-1/2 md:basis-1/3 lg:basis-1/4">
                    <div className="group relative bg-white rounded-3xl  hover:shadow-2xl transition-all duration-500 overflow-hidden border border-amber-100 hover:border-amber-200 hover:-translate-y-2">
                      ...
                      <div className="relative aspect-square p-0 bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-50 overflow-hidden">
                        {/* Decorative Background Pattern */}
                        <div className="absolute inset-0 opacity-10">
                          <div className="absolute top-4 right-4 w-20 h-20 bg-amber-300 rounded-full blur-xl"></div>
                          <div className="absolute bottom-4 left-4 w-16 h-16 bg-orange-300 rounded-full blur-lg"></div>
                        </div>
                        <img
                        ...
                        {/* Enhanced Badge */}
                        {product.badges?.[0] && (
                          <div className="absolute top-3 left-3 bg-gradient-to-r from-red-500 to-red-600 text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-lg border-0 border-white z-20">
                            {product.badges[0]}
                          </div>
                        )}
        ...
        <div className="text-center mt-8">
          <Link to="/shop">
            <Button className="bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white font-semibold py-3 px-8 text-sm rounded-xl shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-300">
              VIEW ALL PRODUCTS
...
export default TrialPacksSection;